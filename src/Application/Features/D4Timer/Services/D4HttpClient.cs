...
public class D4HttpClient : ID4HttpClient
{
    private readonly HttpClient _httpClient;

    public D4HttpClient(HttpClient httpClient)
    {
        _httpClient = httpClient;
        _httpClient.BaseAddress = new Uri(D4TimerConstants.BaseAddress);
    }
    public async Task<D4Boss> GetUpcomingBoss()
    {
        
        var response = await _httpClient.GetAsync(D4TimerConstants.GetUpcomingBoss.Endpoint).ConfigureAwait(false);
